...
class Lyon  {
 ...
 def Process(workPath: String, backupPath: String, sourcesPath: String, rawPath: String, urlfile: String, geoJsonPath: List[(String,String)], spark: SparkSession) = {
  ...
  def UploadAzure(dataframes: List[DataFrame], rawPath: String, spark: SparkSession) = {
    ...
    stop_times.repartition(20).dropDuplicates().write.mode("overwrite").parquet(raw_path + "stop_times.parquet")
    ...
    // Write in raw path
    shapes.repartition(1).dropDuplicates().orderBy("shape_id","shape_pt_sequence").write.mode("overwrite").parquet(raw_path + "shapes.parquet")
