package com.alstom.shapes.GTFSOperations
...
object GTFSMethods {
  ...
  def AddSystemX(dataframes: List[DataFrame], geoJsonPath: String, spark: SparkSession) : List[DataFrame] = {
    ...
    println("Routes === " + routes_newDF.count())
...
*/
  ...
  def RemoveUnwantedRoutes(dataframes: ListBuffer[DataFrame],spark: SparkSession) : List[DataFrame] = {
    ...
    println("Start Remove Type Bus And RailWay Except The Ones Wich Pass Through Lyon Of SNCF")
    ...
    println("Finish Remove Type Bus And RailWay Except The Ones Wich Pass Through Lyon Of SNCF")
